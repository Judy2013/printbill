describe("打印小票", function() {
  ...
  it("有符合买二赠一条件商品", function() {
    invoice = new PrintInvoice(codes, products);
    invoice.printAll();
    expect(invoice.save).toEqual(4);
  });
  it("没有符合买二赠一条件商品", function() {
    ...
    invoice = new PrintInvoice(codes, p);
    invoice.printAll();
    expect(invoice.toPay).toEqual(25);
  });
  it("有95折商品", function() {
    var p = [];
    for (var i = 0; i < products.length; i++) {
    ...
    invoice = new PrintInvoice(codes, p);
    invoice.printAll();
    expect(invoice.toPay).toEqual(24.45);
  });
  it("有买二赠一又有95折商品", function() {
    ...
    invoice = new PrintInvoice(codes, p);
    invoice.printAll();
    expect(invoice.toPay).toEqual(20.45);
  });
});
